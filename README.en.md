...
> One-sentence requirement → design direction → tech stack → working code
...
- ❌ AI generates inconsistent styles every run — no team-wide design alignment
- ❌ Output lacks design systems (typography, color hierarchy, spacing rhythm)
- ❌ No standardized process for accessibility, mobile fallback, or performance
- ❌ "Switching directions" requires rewriting every prompt from scratch
...
（hard contracts）      （the "mind"）          （reusable blueprints）
                         ┌───────────────────────┐
                         │  Composed Prompt       │
                         │  = T1 + T2 + T3 + user │
                         └───────────┬─────────────┘
...
The "laws" — hard contracts every output must follow. Guarantees consistency and a quality floor.
...
| System Architecture Guide | `SYSTEM_PROMPT_ARCHITECTURE.md` | Prompt system design document |
...
### 15 Interaction Signatures

Pick 3–5 per project. This is the "menu" — signature elements that make output look professional.
...
│
├── project-rules/                        ← T1 · Hard contracts
│   ├── artistic-stack.md
│   ├── nextjs-shadcn.md
...
│   └── shared-security.md
│
├── expert-personas/                      ← T2 · AI mindset / Personas
...
│
├── task-templates/                       ← T3 · Blueprint prompts
...
│   └── plugin-i18n.md
│
├── references/                           ← Design DNA & patterns
...
MIT — use, modify, ship. If this helped you build something cool, a star ⭐ is appreciated.
